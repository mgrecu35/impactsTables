...
!
!  Differentiation of suml1b in reverse (adjoint) mode:
!   gradient     of useful results: y
!   with respect to varying inputs: k t y a
!   RW status of diff variables: k:out t:out y:in-zero a:out
SUBROUTINE SUML1B_B(y, yb, lam1, k, kb, a, ab, g, n, t, tb, eps, dz)
  IMPLICIT NONE
  INTEGER :: n
  REAL :: y, lam1(2*n+3), lam2(2*n+3), k(n+1), a(n+1), g(n+1), t(n+1), &
& dz, eps
  REAL :: yb, kb(n+1), ab(n+1), tb(n+1)
  REAL :: b(2*n+3), am, km, gm
  REAL :: bb(2*n+3)
  INTEGER :: i
  REAL :: tempb
  ...
  DO i=n,0,-1
    ...
    tempb = dz*3*bb(2*i+2)
    bb(2*i+2) = 0.0
    ab(i+1) = ab(i+1) - k(i+1)*t(i+1)*tempb
    tempb0 = (1-a(i+1))*tempb
    kb(i+1) = kb(i+1) + t(i+1)*tempb0
    tb(i+1) = tb(i+1) + k(i+1)*tempb0
  END DO
  yb = 0.0
...
& , epsb, dz)
  ...
  REAL :: y, lam1(2*n+3), lam2(2*n+3), k(n+1), a(n+1), g(n+1), t(n+1), &
& dz, eps
  REAL :: yb, kb(n+1), ab(n+1), gb(n+1), epsb
  REAL :: abig(2*n+3, 2*n+3), am, km, gm
...
SUBROUTINE TBF90_B(tb, tbb, i01p, i01pb, t, tb0, incang, k, kb, a, ab, g&
& , gb, n, eps, epsb, dz)
  IMPLICIT NONE
  REAL :: incang, eps, dz
  REAL :: epsb
  ...
  REAL :: tbb
  REAL :: k(n+1), a(n+1), g(n+1)
  REAL :: kb(n+1), ab(n+1), gb(n+1)
  REAL :: i01p(2*n+3)
  REAL :: i01pb(2*n+3)
  REAL :: jup(n+1), jd(n+1), t(n+1)
  REAL :: jupb(n+1), jdb(n+1), tb0(n+1)
  REAL :: cos1
  REAL :: intdo(n+1)
  REAL :: intdob(n+1)
  REAL :: intup(n+1)
...
&     g(k1)*cos1)+i01p(2*k1))
  ...
  temp = -(intup(1)/cos1)
  tempb = EXP(temp1)*tbb
  intupb(1) = intupb(1) - (EXP(temp1)*(1-eps)*sumjd/cos1+EXP(temp)*eps*t&
&   (1)/cos1)*tbb
  sumjub = tbb
  tempb1 = EXP(temp)*tbb
  epsb = t(1)*tempb1 - sumjd*tempb
  tb0(1) = tb0(1) + eps*tempb1
  sumjdb = (1-eps)*tempb
  kb = 0.0
  intdob = 0.0
  jupb = 0.0
...
&     + (cos1*0.5*(temp*g(k1))+i01p(2*k1)-t(k1))*jupb(k1)
    tb0(k1) = tb0(k1) + (1-a(k1))*jdb(k1) + (1-a(k1))*jupb(k1)
